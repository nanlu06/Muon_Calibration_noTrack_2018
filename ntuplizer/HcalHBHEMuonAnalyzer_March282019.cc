...
private:
  ...
  std::vector<bool>         muon_is_tight_, tmpmatch_, trackIDokHCAL_, muon_is_medium_probe_,isMuonRec_;
  ...
  if (_Muon.isValid() && barrelRecHitsHandle.isValid() && 
    ...
    for (reco::MuonCollection::const_iterator RecMuon = _Muon->begin(); RecMuon!= _Muon->end(); ++RecMuon)  {
      ...
      if (RecMuon->innerTrack().isNonnull()) {
        ...
        if ((RecMuon->pt()<20.0) || fabs(ieta)<21 || (!muon_is_tight) || isoR04>0.15) continue;
	...
	h_cutflow->Fill(0); //initial muon selections
        ...
        for (reco::MuonCollection::const_iterator iMuon = _Muon->begin(); iMuon!= _Muon->end(); ++iMuon)  {
         if(iMuon!=RecMuon && (iMuon->charge()!=RecMuon->charge())){
           ...
           bool muon2 = muon::isLooseMuon(*iMuon);
           bool muon3 = (iMuon->pfIsolationR04().sumChargedHadronPt + std::max(0.,iMuon->pfIsolationR04().sumNeutralHadronEt + iMuon->pfIsolationR04().sumPhotonEt - (0.5 *iMuon->pfIsolationR04().sumPUPt))) / iMuon->pt()<0.25;
	...
	h_cutflow->Fill(1); // Zmm cut
        ...
        trackIDokHCAL_.push_back(trackID.okHCAL);
        tmpmatch_.push_back(tmpmatch);
...
void HcalHBHEMuonAnalyzer::beginJob() {
  ...
  tree_->Branch("tmpmatch_muon",                    &tmpmatch_);
  tree_->Branch("trackIDokHCAL_muon",               &trackIDokHCAL_);
